	...
	class commercebird_AM_Client
	{
		...
		public function __construct($file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '', $custom_menu = array(), $inactive_notice = true)
		{
			...
			if ($this->no_product_id) {
				...
				$product_id = strtolower(str_ireplace(
					array(
						' ',
						'_',
						'&',
			 ...
			 */
			$this->data_key = 'wc_am_client_' . strtolower(str_ireplace(
				array(
					' ',
					'_',
					'&',
		...
		public function check_external_blocking()
		{
			...
			if (defined('WP_HTTP_BLOCK_EXTERNAL') && WP_HTTP_BLOCK_EXTERNAL === true) {
				...
				if (!defined('WP_ACCESSIBLE_HOSTS') || stristr(WP_ACCESSIBLE_HOSTS, $host) === false) {
					...
					<div class="notice notice-error">
						<p>
							<?php printf(__('<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', $this->text_domain), $this->software_title, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?>
						</p>
					</div>
					<?php
				}
