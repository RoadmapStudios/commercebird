...
if ( ! class_exists( 'Cmbird_AM_Client' ) ) {
	class Cmbird_AM_Client {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '', $custom_menu = array(), $inactive_notice = true ) {
			 ...
			 */
			$this->data_key            = 'wc_am_client_' . strtolower(
				str_ireplace(
					array(
						' ',
						'_',
			...
			if ( is_admin() ) {
				 ...
				 */
				$this->wc_am_domain           = str_ireplace(
					array(
						'http://',
						'https://',
					),
		...
		public function uninstall() {
			...
			if ( apply_filters( 'wc_am_client_uninstall_disable', true ) ) {
				...
				if ( is_multisite() ) {
					...
					foreach ( array( $this->wc_am_instance_key, $this->wc_am_deactivate_checkbox_key, $this->wc_am_activated_key ) as $option ) {
		...
		public function check_external_blocking() {
			...
			if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					...
					<div class="notice notice-error">
						<p>
							<?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %1$s updates. Please add %2$s to %3$s.', $this->text_domain ), $this->software_title, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?>
						</p>
					</div>
					<?php
				}
		...
		public function config_page() {
			...
			<div class='wrap'>
				<h2>
					<?php esc_html_e( $this->wc_am_settings_title, $this->text_domain ); ?>
				</h2>
		...
		public function load_settings() {
			...
			add_settings_field(
				'info',
				esc_html__( 'Activation Info', $this->text_domain ),
				array(
					$this,
		...
		public function wc_am_deactivate_textarea() {
			...
			<span class="description">
				<?php esc_html_e( 'Deactivates an API Key so it can be used on another blog.', $this->text_domain ); ?>
			</span>
			<?php
		}
