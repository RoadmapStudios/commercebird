...
if ( ! class_exists( 'ZI_CommonClass' ) ) {
	class ZI_CommonClass {
		...
		public function get_woo_tax_class_from_zoho_tax_id( $zoho_tax_id ) {
            ...
            $tax_option_obj = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $option_table WHERE option_value LIKE %s LIMIT 1", array( "$zoho_tax_id##%" ) ) );
			...
			if ( $tax_option_key ) {
				...
				if ( $tax_rate_id ) {
                    ...
                    $tax_rate_obj = $wpdb->get_row( $wpdb->prepare( 'SELECT tax_rate_class FROM %s WHERE tax_rate_id = %d LIMIT 1', $rates_table, $tax_rate_id ) );
