<template>
  <Card :foot="true" :title="`${store.subscription.plan && store.subscription.plan.length > 0 ? store.subscription.plan : 'Your Plan'}`">
    ...
    <div v-if="store.subscription.fee_lines" class="px-4 py-2 space-y-4">
      ...
      <ul class="flex flex-wrap items-center gap-2">
        <li v-for="item in store.subscription.fee_lines" :key="item">
          <span class="inline-flex rounded-md shadow items-center border px-3 py-0.5 text-sm font-medium">{{
              item.name
            }}</span>
        </li>
      </ul>
      ...
      <p class="text-sm leading-relaxed text-gray-600">
        ...
        <span class="font-medium text-gray-900">{{ store.subscription.total }} {{ store.subscription.currency }}</span>
        on
    ...
    <div v-else class="p-4 my-6">
      <a class="relative w-full text-center rounded-lg hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
         href="https://wooventory.com/pricing" target="_blank" type="button">
        <span class="flex items-center justify-center gap-4 mt-2 text-sm font-medium text-gray-900">Subscribe for</span>
        <span class="flex items-center justify-center gap-4 mt-2 text-sm font-medium text-gray-900">
