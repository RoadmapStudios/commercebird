...
import { Bars3Icon, BellAlertIcon, TagIcon, XMarkIcon, } from "@heroicons/vue/24/outline";
...
<template>
  <nav class="bg-white shadow">
    <div class="px-4 mx-auto max-w-7xl sm:px-8">
      <div class="relative flex justify-between h-16">
        <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
          <!-- Mobile menu button -->
          <button aria-expanded="false" type="button" @click.prevent="showMobileMenu = !showMobileMenu">
          ...
          <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
            <router-link :class="{
              ...
              'border-b-2 border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700':
                router.currentRoute.value.path !== '/',
            }" class="inline-flex items-center px-1 pt-1 text-sm font-medium border-b-2 focus:ring-0" to="/">Welcome
            </router-link>
            <router-link :class="{
              'border-b-2 border-indigo-500 text-gray-900':
              ...
              <span v-else class="relative flex w-3 h-3">
                <span class="absolute inline-flex w-full h-full rounded-full opacity-75 bg-rose-400 animate-ping"></span>
        ...
        <div class="absolute inset-y-0 right-0 flex items-center gap-4 pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          <div class="hidden sm:flex">
            <BaseButton :loading="loader.isLoading('clear_cache')" type="lite" @click.prevent="clearCache">
              Clear Cache
            </BaseButton>
          </div>
          <div>
            <Sidebar title="Announcement"
              ...
              <template #content>
                <div class="flow-root">
                  <LoaderIcon v-if="loader.isLoading(backendAction.get_changelog)" />
                  <ul v-if="Object.keys(homepage.changelog).length > 0" class="-mb-8">
                    <li v-for="(item, index) in homepage.changelog" :key="index">
                      <div class="relative pb-8">
                        <span aria-hidden="true" class="absolute top-5 left-5 -ml-px h-full w-0.5 bg-gray-200"></span>
                        <div class="relative flex items-start space-x-3">
                          ...
                          <div class="flex-1 min-w-0">
                            <div>
                              <div class="text-sm">
                                <a :href="item.link" class="font-medium text-gray-900" target="_blank"
                                  v-html="item.title.rendered"></a>
                              </div>
                              <p class="mt-0.5 text-sm text-gray-500">
                                {{ formatDate(item.modified_gmt) }}
                              </p>
                            </div>
                            <div class="mt-2 text-sm text-gray-700" v-html="item.content.rendered"></div>
                          </div>
                        </div>
                      </div>
                    </li>
    ...
    <div v-if="showMobileMenu" id="mobile-menu" class="sm:hidden">
      <div class="space-y-1">
        <router-link :class="{
          ...
          'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700':
            router.currentRoute.value.path !== '/',
        }" class="block p-2 text-base font-medium border-l-4" to="/">Welcome
        </router-link>
        <router-link :class="{
          'bg-indigo-50 border-indigo-500 text-indigo-700':
            router.currentRoute.value.path === '/zoho-inventory',
      ...
      <div class="flex p-2">
        <BaseButton type="lite" @click.prevent="clearCache">Clear Cache
        </BaseButton>
      </div>
    </div>
  </nav>
