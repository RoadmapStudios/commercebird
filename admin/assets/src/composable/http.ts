 ...
 * @param {any} params - The parameters to send with the action.
 * @return {Promise<any>} The data returned from the action, if successful.
 */
export const resetData = async (action: string, storeKey: string, params: any = null): Promise<any> => {
  if (!action) return;

  const requestUrl = ajaxUrl(action);
  const bodyData = params ? { ...params } : {};
  ...
  const response = await fetch(requestUrl, {
    ...
    body: JSON.stringify(bodyData),  // Convert to JSON string
  });
